...
class OrderForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['order_date'].label = 'Дата оформления заказа'
    order_date = forms.DateField(widget=TextInput(attrs={'type':'date'}))

    class Meta:
        model = Order
        fields = (
            'first_name', 'last_name', 'phone', 'address', 'order_type', 'order_date', 'comments'
        )
...
class RegistrationForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        if User.objects.filter(username=username).exists():
            raise forms.ValidationError(f'Имя пользователя {username} занято!')
        return username
